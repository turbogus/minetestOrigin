...
function dom_m.serialize(x)
  ...
  function dump_val(x)
    ...
    elseif t=="userdata" then return x:to_table()
    elseif t=="function" then 
    else
      print("Can't serialize data of type "..t)
      error ""
    end
  end
...
function dom_m.stringtotable(sdata)
...
function dom_m.deserialize(sdata)
  local table = {}
  local okay,results = pcall(dom_m.stringtotable, sdata)
  if okay then
    return results
  end
  print('error:'.. results)
...
local function unit_test()
  ...
  unittest_output = dom_m.deserialize(minetest.serialize(unittest_input))
